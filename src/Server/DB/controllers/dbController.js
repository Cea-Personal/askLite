...
class dbController {
  ...
  static async createUser( req, res ) {
    const createUser = `INSERT INTO users(userId, firstName, lastName, userName, email, bio, stack,
      quote,profileImage, profileImageId, password,password2, isAdmin, userCreated)
  ...
  invalid((result)=>{
      ...
      return res.status(200).json({'Message':'You are logged out',token})
  })
  }catch(error){
  ...
  static async editProfile (req, res){
    ...
    try{
      const values =[
        ...
        rows[0].password || helper.hashPassword(req.body.password),
        new Date(),
        req.user.userId
      ]
...
// delete user
  ...
  static async postQuestion( req, res ) {
    try {
      ...
      const createQuestion = `INSERT INTO questions(questionId,authorId,subject,subjectVector,question,questionVector,authorName,category,tags,questionCreated)
                   VALUES($1,$2,$3,to_tsvector('english',$4), $5, to_tsvector('english',$6),$7,$8,$9,$10)`;
      const values = [
        uuidv4(),
        req.user.userId,
        req.body.subject,
  ...
  static async search(req, res){
    try{
      const searchAll =`Select subject,question from questions
      Where subjectVector @@ to_tsquery('english',$1) OR questionVector @@ to_tsquery('english',$1)`
      const {rows,rowCount} = await client.query(searchAll,[req.query.searchString])
      if (!rows[0]) {
        return res.status(404).json({'message': 'question not found'});
      }
      return res.status(200).json({
 ...
 /*--------------------------Answers Method-------------------------*/ 
  ...
  static async updateAnswer ( req, res ) {
    try {
      const findAnswer =`SELECT * from answers WHERE questionId=$1`
      const findQuestionAuthor =`SELECT authorId from questions Where questionId=$1`
      const questionnaire = await client.query(findQuestionAuthor,[rows[0].questionid])
      ...
      const vote =`UPDATE answers
      SET upvotes=$1,downvotes=$2 WHERE answerId = $3 returning *`
      const votes=[
        req.body.upvotes|| rows[0].upvotes,
        req.body.downvotes|| rows[0].downvotes,
        req.params.answerId
      ]
      const markPreferred = `UPDATE answers
      SET isPreferred=$1 WHERE answerId = $2 returning *`
      
      const {rows} = await client.query(findAnswer,[req.params.questionId])
        if (!rows[0]) {
          return res.status(404).json({
            'message': 'Answer not found'
          });
        }
        if (req.user.userId === questionnaire.rows[0]){
          const mark = await client.query(markPreferred,[req.body.isPreferred,req.params.answerId])
          res.status(200).json({
            'message':'marked',
            'result':mark.rows[0]
          })
        ...
        if (req.user.userId !== authorid){
          const voting = await client.query(vote,votes)
          return res.status(200).json({
            'message':'votes updated',
            'result': voting.rows[0]
          })
        }
  ...
  static async updateComment ( req, res){
    try {
      ...
      const insertComment =`INSERT INTO comments(answerId, authorId, commentAuthor,description,desVector,commentCreated)
      Values($1 , $2 , $3, $4,$5)`
